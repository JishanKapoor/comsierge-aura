...
const BusinessSection = () => {
  ...
  return (
    <section ref={sectionRef} className="py-20 sm:py-24 px-4 sm:px-6 md:px-16 bg-gradient-to-b from-card/30 to-background border-t border-white/5">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-12 sm:mb-16">
          <div>
            <span className={`text-xs uppercase tracking-[0.2em] text-muted-foreground transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
              For Teams
            </span>
            <h2 className={`mt-4 text-2xl sm:text-3xl md:text-4xl font-light text-foreground transition-all duration-700 delay-100 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
              Business Features
            </h2>
          </div>
          <p className={`text-sm text-muted-foreground max-w-md lg:text-right transition-all duration-700 delay-200 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
